...
After pulling or editing the source, run `pnpm sync` (`scripts/deploy.sh`). It installs from the lockfile, rebuilds `dist/`, and restarts `hq.service`; the running app does not pick up changes until then.
