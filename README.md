...
Nasr's history was imported into `data/nasr.sqlite` on 8 September 2026 and the Nasr install has been removed; the `imported_from` setting records where it came from. The one-shot importer has been deleted along with it, so a database restored from a Nasr backup would need importing by hand.

Override database paths with `HQ_DATABASE` (GitHub), `HQ_NASR_DATABASE` (Nasr), and `HQ_CONTENT_DATABASE` (Content). All three default under `data/`. `HQ_NOTES_DATABASE` is the old name for the Content path; it still works and is deprecated. `HQ_UPLOADS_DIR` moves the files Content pages link to; it defaults to `data/uploads`.
